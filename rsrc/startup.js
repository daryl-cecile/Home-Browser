...
document.querySelector('input[type="text"]').addEventListener('keyup', (evt) => {
    if (evt.keyCode === 13) {
        ...
        HomeBrowser.Broadcaster.broadcast('populate suggestions', document.querySelector('input[type="text"]').value);
    }
});
setInterval(() => {
    let hvalue = document.querySelector('.search .url-helper');
    let ivalue = document.querySelector('.search input').value.replace(/&amp;/g, "&");
...
HomeBrowser.Broadcaster.on('set navbar raw value', (rawValue) => {
    document.querySelector('input[type="text"]').value = rawValue;
});
let swipe_directions = {
    x: [],
    y: []
};
let swipe_timeout;
...
document.addEventListener('mousewheel', (e) => {
    if (HomeBrowser.FLAGS.swiping === true) {
        ...
        swipe_timeout = setTimeout(() => {
            ...
            let direction = "";
            if (swipe_directions.x < 0)
                direction = "right";
            if (swipe_directions.x > 0)
                direction = "left";
            if (Math.abs(swipe_directions.x) > 20)
                HomeBrowser.Broadcaster.broadcast('user swipe', direction);
        }, 400);
    }
});
...
HomeBrowser.Broadcaster.on('main update', (data) => {
    if (data === 'resize') {
        ...
        document.querySelectorAll('webview').forEach((value) => value.style.height = newHeight + 'px');
...
HomeBrowser.Broadcaster.on('capture ready', (param) => {
    let t = document.querySelector(`side-bar>tab[data-tab-id="${param.tab_data.id}"] [data-is="thumb"]`);
    ...
    document.querySelectorAll('.vert > span:nth-of-type(2)').forEach((e) => {
        e.style.height = newHeight + "px";
    });
});
require('electron').remote.session.fromPartition('persist:web').protocol.registerFileProtocol('home', ((request, callback) => {
