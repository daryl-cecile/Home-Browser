class TabData {
    constructor(info) {
        ...
        HomeBrowser.Broadcaster.on('url', (data) => {
            if (this.active === true) {
                document.querySelector('.search input').value = data;
            }
        });
...
class TabManager {
    ...
    static closeTab(id) {
        let data = TabManager.dataStore[id];
    ...
    static openTab(id) {
        ...
        if (tabContents.length !== 0) {
            tabContents.forEach(tc => {
                ...
                let data = TabManager.dataStore[parseInt(tc.getAttribute('data-tab-id'))];
                data.active = false;
            });
        }
    ...
    newTab(url = 'about:blank', autoOpen = true) {
        let newTabURL = url;
        if (newTabURL === 'about:blank')
            newTabURL = HomeBrowser.Settings.general.newTabURL;
    ...
    addTab(data, openWhenReady = true) {
        this._tabIncrement++;
        ...
        data.loadWeb(this, () => {
            ...
            data.tabWebElement.View.getWebContents().on('certificate-error', (e, url, error, cert, callback) => {
        ...
        if (openWhenReady === true)
            TabManager.openTab(data.id); // select new tab;
    }
}
TabManager.closedTabs = {};
TabManager.dataStore = {};
